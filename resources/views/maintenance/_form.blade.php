    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div>
            ...
            <div class="mt-1 flex rounded-lg shadow-sm">
                ...
                <input type="number" name="service_kilometers" id="service_kilometers" min="0" value="{{ old(
    'service_kilometers',
    $maintenance?->service_kilometers
) }}" placeholder="e.g. 50000" class="block w-full rounded-lg border-gray-300">
        ...
        <div x-show="status === 'in_progress' || status === 'completed'" x-transition>
            <div>
                ...
                <input type="datetime-local" name="started_at"
                    value="{{ old('started_at', isset($maintenance) && data_get($maintenance, 'started_at') ? data_get($maintenance, 'started_at')->format('Y-m-d\TH:i') : '') }}"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2">
                @error('started_at')
                    <p class="mt-1 text-sm text-red-600">
                        {{ $message }}
        ...
        <div x-show="status === 'completed'" x-transition>
            <div>
                ...
                <input type="datetime-local" name="completed_at"
                    value="{{ old('completed_at', isset($maintenance) && data_get($maintenance, 'completed_at') ? data_get($maintenance, 'completed_at')->format('Y-m-d\TH:i') : '') }}"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2">
                @error('completed_at')
                    <p class="mt-1 text-sm text-red-600">
                        {{ $message }}
