...
<div x-data="{
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div>
            ...
            <select name="vehicle_id" id="vehicle_id" x-model="vehicleId" @change="loadSchedules()"
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title', data_get($maintenance, 'title', '')) }}"
                placeholder="e.g. Engine Repair" class="w-full rounded-lg border border-gray-300 px-3 py-2" required>
        ...
        <div>
            ...
            <select name="type" id="type" x-model="type" class="mt-1 block w-full rounded-lg border-gray-300" required>
                <option value="">
        ...
        <div x-show="type === 'preventive'" x-cloak>
            ...
            <select name="maintenance_schedule_id" id="maintenance_schedule_id"
                class="mt-1 block w-full rounded-lg border-gray-300" :required="type === 'preventive'">
                <option value="">
                    Select preventive maintenance schedule
                </option>

                <template x-for="schedule in schedules" :key="schedule.id">
                    <option :value="schedule.id" x-text="schedule.title"
                        :selected="schedule.id == '{{ $selectedScheduleId }}'"></option>
                </template>
            </select>
            ...
            <div x-show="loadingSchedules" class="mt-2 text-sm text-gray-500">
        ...
        <div>
            ...
            <input type="number" name="service_kilometers" id="service_kilometers" value="{{ old(
    'service_kilometers',
    $maintenance?->service_kilometers
        ...
        <div>
            ...
            <select name="status" id="status" x-model="status"
                ...
                @foreach($statuses as $statusOption)
                            <option value="{{ $statusOption->value }}" @selected(
                                old(
                                    'status',
                                    $maintenance?->status?->value ?? 'pending'
                                ) === $statusOption->value
        ...
        {{-- Reported At --}}
        <div>
            <label for="reported_at" class="mb-2 block text-sm font-medium">
                Reported At
            </label>
    ...
    : now()->format('Y-m-d\TH:i')
        ...
        <div x-show="status === 'in_progress' || status === 'completed'" x-transition>
            <label for="started_at" class="mb-2 block text-sm font-medium">
                Started At
            </label>
    ...
    : ''
        ...
        <div x-show="status === 'completed'" x-transition>
            <label for="completed_at" class="mb-2 block text-sm font-medium">
                Completed At
            </label>
    ...
    : ''
        ...
        {{-- Service Provider --}}
        <div>
            <label for="service_provider" class="mb-2 block text-sm font-medium">
                Service Provider
            </label>
